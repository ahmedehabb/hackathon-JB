...
sleep 1.3  # Wait for 2 seconds to give Chromium time to load properly

# 5. Start recording screen (1280x720 at top-left) in background
echo "🎥 Starting screen recording..."
ffmpeg -y -f avfoundation -framerate 30 -t 5 -video_size 1280x720 -i "Capture screen 0:1" output.mp4 > /dev/null 2>&1 &
FFMPEG_PID=$!
